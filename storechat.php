...
<?php
$f = "true"; //"placeholder='Add a chat user' disabled";
if (!isset($_SESSION))
    session_start();
if (!isset($_COOKIE['contact_alias'])) {
    setcookie("contact_alias","with others!");
    $f = "false";
...
$chat .= '<tr><td><b style="font-size:15px;color:red">Cheri with ' . $_COOKIE['contact_alias'] . '</b> : : </td>';
...
$chat .= '<input contentEditable="' . $f . '" spellcheck="true" onkeypress=\'goChat(this,event.keyCode)\' style="font-size:24px;border:2px solid darkblue;width:250px;" type="text"></div></td></tr></table>';
$chat .= '</div>';
$g = str_replace('"',"'",$chat);
echo json_encode($g);
?>